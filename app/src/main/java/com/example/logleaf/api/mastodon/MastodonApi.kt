...
class MastodonApi(private val sessionManager: SessionManager) {
    ...
    private fun determineSinceDate(account: Account.Mastodon): String? {
        return when {
            // 「全期間」の場合は常にnull（制限なし）
            account.period == "全期間" -> {
                Log.d("MastodonApi", "全期間設定のため制限なしで取得")
                null
            }
            ...
            account.period != "全期間" -> {
                ...
                account.lastSyncedAt?.let { lastSync ->
                    ...
                    if (lastSyncDateTime.isAfter(periodDateTime)) {
                        Log.d("MastodonApi", "差分取得: 前回同期時刻($lastSync)を使用")
                        lastSync
                    } else {
                        Log.d(
                            "MastodonApi",
                            "期間変更: 期間指定(${account.period} -> $periodSinceDate)を使用"
                        )
                        periodSinceDate
                    }
                } ?: run {
                    Log.d(
                        "MastodonApi",
                        "初回同期: 期間指定(${account.period} -> $periodSinceDate)を使用"
                    )
                    periodSinceDate
                }
            }
            ...
            // その他の場合（念のため）
                ...
                Log.d("MastodonApi", "フォールバック: 前回同期時刻を使用")
